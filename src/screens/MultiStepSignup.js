...
const MultiStepSignup = ({ onSignupSuccess, onSwitchToLogin }) => {
  ...
  const handleFinalSignup = async (finalData) => {
    ...
    try {
      const API_BASE_URL = 'http://192.168.1.193:5000'; // Replace with your IP
      ...
      if (response.ok) {
        // Success - pass user data to parent and trigger tutorial flow
        ...
        // Call onSignupSuccess with user data to trigger tutorial
      ...
      // Handle error (maybe show an error screen or go back to step 1)
