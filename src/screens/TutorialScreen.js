...
const TutorialSongCard = ({ song, style, onSwipeLeft, onSwipeRight, isTop = false, isDemo = false }) => {
  ...
  const handlePlayPause = () => {
    if (isDemo) return; // Disable play/pause in demo mode
...
const TutorialScreen = ({ user, onComplete }) => {
  ...
  const handleNext = () => {
    if (currentSlide < tutorialSlides.length - 1) {
      ...
      if (tutorialSlides[currentSlide + 1].showCards) {
        setSongs([...TUTORIAL_SONGS]); // Reset songs for demo
      }
    } else {
      onComplete();
    }
  ...
  const handleSwipeRight = (song) => {
    ...
    setTimeout(() => {
      if (songs.length <= 1) {
        setSongs([...TUTORIAL_SONGS]); // Reset for continuous demo
      }
    }, 300);
  };
  ...
  // Auto-demo animation for instruction slides
  useEffect(() => {
